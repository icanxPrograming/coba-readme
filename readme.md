...
### Error: Middleware [role:owner] not found

**Penyebab:** Middleware Spatie belum didaftarkan

**Solusi:** Daftarkan middleware Spatie di `bootstrap/app.php`
...
### Error: View [products.stock-history] not found

**Penyebab:** File view belum dibuat

**Solusi:** Buat file `stock-history.blade.php` di `resources/views/products/`

### Error: Method show does not exist

**Penyebab:** Method `show()` tidak ada di controller

**Solusi:** Tambahkan method `show()` di controller yang bersangkutan

### Error: CSRF token mismatch

**Penyebab:** Session expired atau cache bermasalah

**Solusi:** Clear cache browser atau jalankan:
...
### Error: 403 Forbidden saat transaksi

**Penyebab:** User tidak memiliki permission yang diperlukan

**Solusi:** 
1. Pastikan user memiliki role yang sesuai (Kasir, Manager, atau Owner)
2. Cek permission dengan tinker: `$user->hasPermissionTo('create transactions')`

### Error: Unexpected token '<' (JSON parsing error)

**Penyebab:** Server mengembalikan HTML error, bukan JSON

**Solusi:** 
1. Cek response di Network tab (F12 → Network)
2. Pastikan route mengembalikan response JSON untuk AJAX request
3. Cek log error di `storage/logs/laravel.log`

### Reset Password Default

Jika user lupa password, owner bisa reset password massal per cabang:

1. Buka menu **Cabang**
2. Klik ikon **titik tiga (...)** pada cabang yang dituju
3. Pilih **Reset Semua Password**
4. Password baru untuk semua akun di cabang tersebut: `password123`
5. User akan diwajibkan mengganti password saat login berikutnya

### Clear Cache Komprehensif
...
$cancelled = App\Models\Transaction::onlyTrashed()->get();
$cancelled->count();
